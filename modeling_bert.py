...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if getattr(config, 'static_sparsity', False):
            sparsity_mask = build_static_sparsity_mask(
                json_file=config.static_sparsity, 
    ...
    def forward(self, hidden_states, attention_mask, *args, **kwargs):
        ...
        #         # code: https://pytorch.org/docs/stable/_modules/torch/nn/modules/normalization.html#LayerNorm
        #         query_layer = F.layer_norm(query_layer, [self.attention_head_size,], weight=None, bias=None)
        #         # [b, h, l, d]
        #         key_layer = key_layer.permute(0, 1, 3, 2)
        #         key_layer = F.layer_norm(key_layer, [self.attention_head_size,], weight=None, bias=None)
        #         key_layer = key_layer.permute(0, 1, 3, 2)
        #         # [b, h, d, l]

        #     elif self.config.normalize_qk == 'normalize':
        #         # doc: https://pytorch.org/docs/stable/nn.functional.html?highlight=norm#torch.nn.functional.normalize
        #         # Performs $L_p$ normalization of inputs over specified dimension.
        #         query_layer = F.normalize(query_layer, p=2, dim=-1)
        #         key_layer = F.normalize(key_layer, p=2, dim=-2)
        #     else:
        #         attention_scores = torch.matmul(query_layer, key_layer)
        #         with torch.no_grad():
        #             query_norm = query_layer.norm(dim = -1, keepdim=True)
        #             key_norm = key_layer.norm(dim = -2, keepdim=True)
        #             query_layer = query_layer/query_norm
        #             key_layer = key_layer/key_norm

        # if hasattr(self, 'quant_matmul'):
        #     quant_attention_scores = self.quant_matmul(query_layer, key_layer)
        #     if self.config.normalize_qk == 'inner_product':
        ...
        # #attention_scores = torch.matmul(query_layer, key_layer)*query_norm*key_norm
        # if not self.config.normalize_qk == 'inner_product':
        #     attention_scores = torch.matmul(query_layer, key_layer)
        
        # attention_scores = attention_scores / math.sqrt(self.attention_head_size)
        # quant_attention_scores = quant_attention_scores / math.sqrt(self.attention_head_size)
        # if getattr(self.config, 'prune_score', False):
        #     threshold = self.config.prune_score['threshold']
        #     if quant_attention_scores is not None:
        #         sparsity_mask = gen_sparsity_mask(threshold, quant_attention_scores, attention_mask)
        #     else:
        #         sparsity_mask = gen_sparsity_mask(threshold, attention_scores, attention_mask)

        #     attention_mask =  attention_mask + sparsity_mask

        do_static_sparsity = getattr(self.config, 'static_sparsity', False)
        if do_static_sparsity:
        ...
        #     raise NotImplementedError('sparsemax and entmax are not implemented yet')
        # else:
